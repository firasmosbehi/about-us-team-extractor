...
const ABOUT_POSITIVE_PATTERNS = [
  { pattern: 'about us', weight: 30 },
  { pattern: 'about-us', weight: 30 },
  { pattern: 'about', weight: 20 },
  { pattern: 'company', weight: 18 },
  { pattern: 'our company', weight: 20 },
  { pattern: 'who we are', weight: 24 },
  { pattern: 'who-we-are', weight: 24 },
  { pattern: 'our story', weight: 18 },
  { pattern: 'our-story', weight: 18 },
  { pattern: 'mission', weight: 14 },
  { pattern: 'values', weight: 14 },
  { pattern: 'culture', weight: 12 },
  { pattern: 'why us', weight: 10 },
  { pattern: 'overview', weight: 8 }
];
...
function scoreAgainstPatterns(s, positivePatterns) {
  let score = 0;
...
export function rankTeamPageCandidates({ anchors, baseUrl, maxCandidates }) {
  ...
  for (const a of anchors || []) {
    ...
    let score = 0;
    score += scoreAgainstPatterns(text, POSITIVE_PATTERNS);
    score += scoreAgainstPatterns(hrefNorm, POSITIVE_PATTERNS);
...
export function rankAboutPageCandidates({ anchors, baseUrl, maxCandidates }) {
  const base = new URL(baseUrl);
  const rootDomain = stripWww(base.hostname);
  ...
  for (const a of anchors || []) {
    ...
    let score = 0;
    score += scoreAgainstPatterns(text, ABOUT_POSITIVE_PATTERNS);
    score += scoreAgainstPatterns(hrefNorm, ABOUT_POSITIVE_PATTERNS);
  ...
  const seen = new Set();
  const out = [];
  const limit = Math.max(0, Math.min(Number(maxCandidates) || 2, 10));
